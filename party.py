...
def is_mel(name, email):
    ...
    True
    >>> is_mel('1m6 e*l', 'mel@ubermelon.com')
    True
    """
    name.join([letter for letter in name if letter.isalpha()==True]).title()
    # name_list = [letter for letter in name if letter.isalpha()==True]
    # name.join(name_list).title()
    # name = name.title()
    mel_variations = ['Mel', 'Melitpolski', "Melmelitpolski"]
    email = email.lower()

    return name in mel_variations or email == "mel@ubermelon.com"
